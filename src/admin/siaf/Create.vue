<template>
	<v-form header="Generar Hoja de Control Contable" action="/api/voting/act">
		...
		<div class="v-scrollable">
			<v-table ref="t1" v-bind:value="o.header" selectable="false" scrollable="true"
				style="height:300px;margin-bottom:10px;" v-on:row-select="rs = $event.current; $refs.t2.kc++">
			...
			{{ rs }}
			<div v-show="rs">
				<v-table ref="t2" scrollable="true" v-bind:value="selectedRows" v-on:row-select="rs2 = $event.current"
					selectable="false">
					<template v-slot:columns="">
						<td header="Ciclo" width="60" class="center"></td>
						<td header="Fase" width="60" class="center"></td>
...
export default _.ui({
	...
	methods: {
		...
		hcc() {
			var me = this, o = { ...me.rs, ...me.rs2 };//Vue.mergeDeep(me.rs, me.rs2);
		...
		dd() {
			var me = this, hcc = [];
			let montoTotal = 0.0;
			console.log(me.$refs.t2.data)
			me.$refs.t2.data.forEach((rs2) => {
				//const o = Vue.mergeDeep(me.rs, rs2);
				const o = { ...me.rs, ...rs2 };
			...
			fo.append(
				"file",
				new Blob([JSON.stringify(hcc)], { type: "text/plain" }),
				"data.json"
			);
			fo.append("filename", "data.json");
			fo.append("template", "HCC");
